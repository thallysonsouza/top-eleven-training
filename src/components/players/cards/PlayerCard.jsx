...
import { Pencil, Trash2, CalendarSync, RotateCcw, Check } from "lucide-react";
...
function PlayerCard({
  ...
  return (
    <div className={`player-card ${player.checked ? "checked" : ""}`}>
      <button className="player-card-main" onClick={() => onOpen(player)}>
        <h3 title={player.name}>{player.name}</h3>

        <div className="player-overall">
          <span className="player-stars">{stars}</span>

          <strong>{overall.toFixed(1)}</strong>
        </div>

        <div className="player-positions">{positions.join(" • ")}</div>
      </button>
