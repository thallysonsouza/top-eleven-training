...
import { increasePlayer, decreasePlayer } from "../services/playerService";
...
function FootballField({
  ...
  function handleRemovePlayer(position) {
    setLineup((previous) => removePlayer(previous, position));
  }
  ...
  const availablePlayers = selectedPosition
    ? team.players.filter((player) => {
        ...
        return (
          playerPositions.includes(getBasePosition(selectedPosition)) &&
    ...
    <>
      <section className="football-field">
        ...
        {fieldLayout
          ...
          .map((position, index) => {
            if (!position) {
              return <div key={index} className="football-empty" />;
            }

            const player = lineup[position];

            const hideSlot = lineupCount === 11 && !player;

            if (hideSlot) {
              return <div key={position} className="football-empty" />;
            }
            ...
            return (
              <PositionSlot
                ...
                onIncrease={() => handleIncreasePlayer(player)}
                onDecrease={() => handleDecreasePlayer(player)}
              />
            );
          })}
      </section>
